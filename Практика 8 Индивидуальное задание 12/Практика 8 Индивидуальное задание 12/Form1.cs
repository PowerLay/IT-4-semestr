...
namespace Практика_8_Индивидуальное_задание_12
{
    public partial class Form1 : Form
    {
        ...
        private void CalcFunction()
        {
            ...
            for (var i = 0; i < count; i++)
            {
                ...
                _y[i] = Math.Pow(Math.E, _x[i] - Math.Sqrt(_x[i])) / (Math.Log((_x[i] + 5) / (_x[i] + 1)) -
                                                                      2 * (Math.Pow(_x[i], 3) + _x[i]) /
                                                                      (1 + Math.Abs(Math.Sin(_x[i]))));
            }
        }
