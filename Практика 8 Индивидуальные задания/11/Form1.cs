    ...
    public partial class Form1 : Form
    {
        ...
        private void CalcFunction()
        {
            ...
            for (var i = 0; i < count; i++)
            {
                ...
                if (_x[i] <= 0 || 3 + Math.Log(_x[i]) + 15 - _x[i] < 0 || 1 + Math.Sin((2 + Math.Pow(_x[i], 2)) / (1 + _x[i])) == 0)continue;

                // Вычисляем значение функций в точке X
                _y[i] =Math.Sqrt(3 + Math.Log(_x[i]) + 15 - _x[i]);
                _y[i] /= 1 + Math.Sin((2 + Math.Pow(_x[i], 2)) / (1 + _x[i]));
            }
        }
