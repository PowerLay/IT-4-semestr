    ...
    public partial class Form1 : Form
    {
        ...
        private void CalcFunction()
        {
            ...
            for (var i = 0; i < count; i++)
            {
                ...
                if (_x[i] <= 0 || Math.Abs(_x[i] - 0.0421113) < 0.001 || Math.Abs(_x[i] - 0.651966) < 0.001 || Math.Abs(_x[i] - 10.9269) < 0.001)continue;

                // Вычисляем значение функций в точке X
                _y[i] = Math.Sqrt(2 + 0.5 * Math.Pow(_x[i], 2) + Math.Pow(Math.E, _x[i] + 2));
                _y[i] /= Math.Pow(_x[i], 3) - 3 * Math.Pow(2 * _x[i] - 1, 2);
                _y[i] += Math.Sqrt(_x[i]);
            }
        }
