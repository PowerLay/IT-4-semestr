    ...
    public partial class Form1 : Form
    {
        ...
        private void CalcFunction()
        {
            ...
            for (var i = 0; i < count; i++)
            {
                ...
                if (_x[i] <= 0 || Math.Abs(1 - Math.Pow(Math.Sin(_x[i]), 3)) == 0 ||
                    1 + Math.Pow(Math.E, Math.Sqrt(_x[i])) + Math.Cos(_x[i] * _x[i]) <= 0)
                    continue;


                // Вычисляем значение функций в точке X
                _y[i] = Math.Sqrt(1 + Math.Pow(Math.E, Math.Sqrt(_x[i])) + Math.Cos(_x[i] * _x[i]));
                _y[i] /= Math.Abs(1 - Math.Pow(Math.Sin(_x[i]), 3));
                _y[i] += Math.Log(2 * _x[i]);
            }
        }
