    ...
    public partial class Form1 : Form
    {
        ...
        private void CalcFunction()
        {
            ...
            for (var i = 0; i < count; i++)
            {
                ...
                if (_x[i] <= 0 || Math.Cos(2*_x[i]*_x[i])+Math.Abs(_x[i]-3)==0||Math.Log(_x[i])+2==0)continue;

                // Вычисляем значение функций в точке X
                _y[i] = (Math.Sqrt(_x[i] * _x[i] * _x[i]) - Math.Log(_x[i])) /
                        (Math.Cos(2 * _x[i] * _x[i]) + Math.Abs(_x[i] - 3));
                _y[i] += 3 * Math.Pow(_x[i] - 2, 2) / (Math.Log(_x[i]) + 2);
            }
        }
