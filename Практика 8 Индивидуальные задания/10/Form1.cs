    ...
    public partial class Form1 : Form
    {
        ...
        private void CalcFunction()
        {
            ...
            for (var i = 0; i < count; i++)
            {
                ...
                if (_x[i] <= 0 || Math.Sqrt(3 * _x[i] + 4) - Math.Pow(2, 0.5 * _x[i] - 1) == 0) continue;

                // Вычисляем значение функций в точке X
                _y[i] = Math.Abs(1 + Math.Cos(Math.Sqrt(_x[i])) - Math.Pow(Math.Log(2 * _x[i]), 2));
                _y[i] /= Math.Sqrt(3 * _x[i] + 4) - Math.Pow(2, 0.5 * _x[i] - 1);
            }
        }
