    ...
    public partial class Form1 : Form
    {
        ...
        private void CalcFunction()
        {
            ...
            for (var i = 0; i < count; i++)
            {
                ...
                if (_x[i] == 0 || Math.Abs(_x[i] - (-2.00045)) < 0.001 || Math.Abs(_x[i] - 1.13962) < 0.001)
                    continue;

                // Вычисляем значение функций в точке X
                _y[i] =3 * (Math.Pow(_x[i], 2) - Math.Abs(3 * _x[i])) + Math.Abs(1 - Math.Cos(Math.Pow(_x[i], 2)));
                _y[i] /= 2 * Math.Pow(Math.E, 3 * _x[i] - 2) - 3 * ((2 + _x[i]) / _x[i]);
            }
        }
